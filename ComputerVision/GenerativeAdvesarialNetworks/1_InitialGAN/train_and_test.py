...
class SampleImageCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        generated = self.model_ref.generator(self.fixed_noise, training=False)
...
def build_model(strategy, dataset_type, channels):
    ...
    batch_size = BATCH_SIZE // 4 if is_large else BATCH_SIZE
    input_shape = (
        IMAGE_SIZE[0] * 4,
        IMAGE_SIZE[1] * 4,
        channels,
...
def main():
    ...
    dataset = Dataset()
    train_ds, _, channels = dataset.load_data(args.type)
