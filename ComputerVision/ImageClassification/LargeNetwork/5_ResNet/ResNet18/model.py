...
def conv3x3(x,out_planes,stride=1,name=None):
    ...
    x = tf.keras.layers.Conv2D(out_planes,kernel_size=3,strides=stride,kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2., mode='fan_out', distribution='untruncated_normal'),name=f'{name}_conv')(x)
    return x
...
def make_layer(x, planes, blocks, stride=1, name=None):
    ...
    if stride != 1 or inplanes != planes:
        downsample = [
            tf.keras.layers.Conv2D(filters=planes, kernel_size=1, strides=stride, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2., mode='fan_out', distribution='untruncated_normal'), name=f'{name}.0.downsample.0'),
...
def resnet18_model(input_shape = (INPUT_SIZE[0],INPUT_SIZE[1],3),num_classes=10):
    ...
    x = tf.keras.layers.Conv2D(64, kernel_size=7, strides=2, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2., mode='fan_out', distribution='untruncated_normal'))(x)
    ...
    outputs = tf.keras.layers.Dense(num_classes, kernel_initializer='glorot_uniform', dtype='float32', name='fc')(x)
