...
def conv1x1(x, out_planes, stride=1, name=None):
    return tf.keras.layers.Conv2D(out_planes, kernel_size=1, strides=stride, use_bias=False,
                                  kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2., mode='fan_out', distribution='untruncated_normal'), name=f'{name}_conv')(x)
    ...
    if stride != 1 or inplanes != planes * 4:
        downsample = [
            tf.keras.layers.Conv2D(filters=planes * 4, kernel_size=1, strides=stride, use_bias=False,
                                   kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2., mode='fan_out', distribution='untruncated_normal'), name=f'{name}.0.downsample.0'),
...
def resnet101_model(input_shape=(INPUT_SIZE[0], INPUT_SIZE[1], 3), num_classes=10):
    ...
    x = tf.keras.layers.Conv2D(64, kernel_size=7, strides=2, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2., mode='fan_out', distribution='untruncated_normal'), use_bias=False)(x)
