"""ResNeXt50_32x4d. TensorFlow implementation; no pretrained weights."""
...
def resnext50_32x4d_model(input_shape=tuple(INPUT_SIZE), num_classes=10):
    ...
    for stage, (planes, blocks) in enumerate(zip((64, 128, 256, 512), [3, 4, 6, 3])):
        for block in range(blocks):
            x = bottleneck(x, planes, 2 if stage and block == 0 else 1,
                           32, 4, False, False, f'stage{stage+1}_block{block+1}')
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    if False:
        x = tf.keras.layers.Dropout(.2)(x)
    outputs = tf.keras.layers.Dense(num_classes, activation='softmax', dtype='float32')(x)
    return tf.keras.Model(inputs, outputs, name='resnext50_32x4d_model')
