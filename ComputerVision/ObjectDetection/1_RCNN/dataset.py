    ...
    if split == "train":
        ...
        for class_name in class_names:
            for xml_path in sorted((ann_root / class_name).glob("*.xml")):
                ...
                if boxes:
                    samples.append({"image_id": xml_path.stem, "image_path": str(image_path), "width": width, "height": height, "boxes": np.asarray(boxes, dtype=np.float32), "labels": np.asarray(labels, dtype=np.int32), "areas": np.asarray(areas, dtype=np.float32), "file_name": image_path.name, "split": split})
    else:
...
def collect_voc_detection_samples(voc_root, split):
    """Convert normalized VOC adapter boxes to the pixel geometry used by RCNN."""
    if voc_root is None:
...
class RCNNProposalDataset:
    def __init__(
        ...
        if dataset_name == "imagenet":
            self.samples = collect_imagenet_detection_samples(self.coco_root, "train" if self.split == "train" else "val")
        elif dataset_name == "voc":
            self.samples = collect_voc_detection_samples(self.coco_root, self.split)
        else:
            self.samples = collect_coco_samples(coco_root=self.coco_root, split=self.split)
        self.samples, self.proposal_cache = cached_records("RCNN", dataset_name, split, coco_root, {"input_size": self.input_size, "max_proposals": max_proposals, "proposals_per_image": proposals_per_image, "positive_fraction": positive_fraction, "positive_iou": positive_iou_threshold, "negative_iou": negative_iou_threshold}, self.samples)
        print(
            f"[RCNNDataset2] dataset={self.dataset_name} split={self.split} samples={len(self.samples)} "
            f"proposals_per_image={self.proposals_per_image} (cached metadata; randomized ROI sampling)"
        )
    ...
    def _generator(self):
        ...
        for idx in indices:
            ...
            records = self._build_sample(sample)
            if records is None:
                continue
            image_path = records["image_path"]
            for box, label, bbox_target, bbox_weight in zip(
                records["boxes"], records["labels"], records["bbox_targets"], records["bbox_weights"],
            ):
                yield (
                    image_path.encode(),
...
def collect_coco_classification_samples(coco_root=COCO_ROOT, split=TRAIN_SPLIT):
    ...
    for sample in detection_samples:
        ...
        samples.append({"image_path": sample["image_path"], "label": int(sample["labels"][label_index]) - 1})
...
def build_train_dataset(task_name="detector", dataset_name="coco", root=None, split=None):
    ...
    dataset = RCNNProposalDataset(
        coco_root=root or (IMAGENET_ROOT if dataset_name == "imagenet" else VOC_ROOT if dataset_name == "voc" else COCO_ROOT),
        split=split or ("train" if dataset_name in {"imagenet", "voc"} else "train2017"),
        batch_size=BATCH_SIZE,
        input_size=INPUT_SIZE,
        proposals_per_image=TRAIN_PROPOSALS_PER_IMAGE,
        augment=True,
...
def build_val_dataset(task_name="detector", dataset_name="coco", root=None, split=None):
    ...
    dataset = RCNNProposalDataset(
        coco_root=root or (IMAGENET_ROOT if dataset_name == "imagenet" else VOC_ROOT if dataset_name == "voc" else COCO_ROOT),
        split=split or ("val" if dataset_name in {"imagenet", "voc"} else "val2017"),
        batch_size=BATCH_SIZE,
        input_size=INPUT_SIZE,
