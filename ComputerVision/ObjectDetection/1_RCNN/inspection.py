...
def _boxes(image, boxes, labels, names, prediction=False):
    canvas=image.copy(); color=(0,220,0) if prediction else (0,0,255)
    for box,label in list(zip(boxes,labels))[:20]:
        label,score=label if prediction else (label,None);x1,y1,x2,y2=map(int,box);text=f"{_name(names,label)}: {float(score):.2f}" if prediction else f"GT {_name(names,label)}"
...
class DetectionInspectionCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self,epoch,logs=None):
        ...
        for sample in self.samples:
            ...
            canvas=_boxes(image,sample["boxes"],sample["labels"],self.class_names);detections,_=self.infer_image(self.model,sample["image_path"],self.class_names)
            panels.append(cv2.resize(_boxes(canvas,[d["box"] for d in detections],[(d["class_id"],d["score"]) for d in detections],self.class_names,True),(320,240)))
