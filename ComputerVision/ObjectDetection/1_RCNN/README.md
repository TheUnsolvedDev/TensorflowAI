...
With `--full-reports`, detector training writes `logs/<dataset>/RCNN/inspection/epoch_###.png`: a fixed 4x4 validation grid with red `GT <class>` boxes and green `<class>: <probability>` predictions. Classifier-only runs do not create these grids.

## Runtime defaults

Detector training uses 256x256 inputs and a conservative global batch size 8. Set `DETECTOR_BATCH_SIZE=N` only after measuring a real training step; it must divide evenly across visible replicas. Input records stream through shuffle, batch, and prefetch; proposal targets are regenerated per pass and no cache directory is used. The trainer enables memory growth, uses NCCL `MirroredStrategy` when two or more GPUs are visible, and otherwise falls back to one GPU or CPU. `--resume` preserves checkpoints and fails if their shapes are incompatible.

This directory contains a TensorFlow implementation of a classic R-CNN style object detector. COCO 2017, Pascal VOC 2012, and ImageNet CLS-LOC run as isolated detector datasets with their own logs and checkpoints. Run only VOC with `python train.py --detector voc`; it uses VOC's labelled `train.txt` and `val.txt` splits.
...
```bash
python ObjectDetection/1_RCNN/train.py --gpu 0
```

Use all visible GPUs:

```bash
python ObjectDetection/1_RCNN/train.py --gpu -1
```

Resume training:

```bash
python ObjectDetection/1_RCNN/train.py --gpu 0 --resume
```

Override epochs:

```bash
python ObjectDetection/1_RCNN/train.py --gpu 0 --epochs 5
```
...
```text
ObjectDetection/1_RCNN/logs/coco2017/RCNN/
```
...
```bash
python ObjectDetection/1_RCNN/test.py --gpu 0 --image /path/to/image.jpg
```

### Dataset Split Inference

```bash
python ObjectDetection/1_RCNN/test.py --gpu 0 --num_images 5
```
...
```text
ObjectDetection/1_RCNN/logs/coco2017/RCNN/inference/
```
...
```bash
python ObjectDetection/1_RCNN/train.py --help
```

Inference help:

```bash
python ObjectDetection/1_RCNN/test.py --help
```
