...
from config import BATCH_SIZE, CHECKPOINT_PATH, CLASSIFIER_CHECKPOINT_PATH, CLASSIFIER_HISTORY_PATH, CLASSIFIER_LOG_DIR, CLASSIFIER_STATE_PATH, COCO_ROOT, EPOCHS, HISTORY_PATH, IMAGENET_ROOT, LOG_DIR, STATE_PATH, STEPS_PER_EPOCH, TRAIN_SPLIT, VALIDATION_STEPS, VAL_SPLIT, VOC_ROOT, ensure_dir
...
class CompactProgress(tf.keras.callbacks.Callback):
    """Keep the slowest training step visible without Keras' wide metric table."""
    def on_epoch_begin(self, epoch, logs=None):
        self.epoch, self.started = epoch + 1, time.monotonic()
        self.steps = self.params.get("steps")

    def on_train_batch_end(self, batch, logs=None):
        step = batch + 1
        if step % 10 and step != self.steps:
            return
        loss = float((logs or {}).get("loss", float("nan")))
        elapsed = time.monotonic() - self.started
        eta = elapsed * (self.steps - step) / step if self.steps else 0.0
        print(f"\rE{self.epoch} {step}/{self.steps} loss={loss:.3f} eta={int(eta // 60)}:{int(eta % 60):02d}", end="", flush=True)
...
def save_model_report(model, task_name):
    ...
    with open(os.path.join(root, f"{task_name}_model_summary.txt"), "w", encoding="utf-8") as file:
        model.summary(expand_nested=True, show_trainable=True, print_fn=lambda line: file.write(f"{line}\n"))
    try:
        tf.keras.utils.plot_model(model, to_file=os.path.join(root, f"{task_name}_model.png"), show_shapes=True, show_trainable=True, expand_nested=True)
    except (ImportError, OSError) as error:
        print(f"[model report] diagram unavailable: {error}")
...
def fit_task(strategy, task_name, dataset_name, gpu_id, continue_training, epochs, initialize_from=None, reset=False, full_reports=False):
    ...
    if not reset and os.path.exists(checkpoint_path):
        try:
            ...
            initial_epoch = load_state(state_path) if continue_training else 0
            print(f"[checkpoint] {'Continuing' if continue_training else 'Warm-started'} matching weights from {checkpoint_path}.")
    elif reset:
        print("[checkpoint] --reset replaced the selected task/dataset artifacts; using new weights at epoch 0.")

    callbacks = [
        CompactProgress(),
        tf.keras.callbacks.ModelCheckpoint(checkpoint_path, save_weights_only=True, verbose=0),
        tf.keras.callbacks.TensorBoard(log_dir=log_dir),
        HistorySaver(history_path),
        StateSaver(state_path),
    ]
    if task_name == "detector":
        callbacks.append(DetectionInspectionCallback(val_builder.samples, os.path.join(log_dir, "inspection"), inspection_class_names(dataset_name), infer_image))
    history = model.fit(
        train_dataset,
...
def main():
    ...
    parser.add_argument("--cache-max-samples", type=int, default=2**22, help="CPU cache capacity; it must cover the complete selected split.")
    ...
    if args.classifier is not None:
        classifier_checkpoint = fit_task(strategy, "classifier", args.classifier, args.gpu, args.continue_training, args.epochs or CLASSIFIER_EPOCHS, reset=args.reset, full_reports=args.full_reports)

    if args.detector is not None:
        fit_task(strategy, "detector", args.detector, args.gpu, args.continue_training, args.epochs or EPOCHS, initialize_from=classifier_checkpoint, reset=args.reset, full_reports=args.full_reports)
