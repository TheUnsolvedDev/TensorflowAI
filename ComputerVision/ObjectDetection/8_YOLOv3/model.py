...
from config import ANCHORS, IMAGENET_NUM_CLASSES, INPUT_SIZE, LEARNING_RATE, MAX_DETECTIONS, NMS_IOU_THRESHOLD, SCALES, SCORE_THRESHOLD, WEIGHT_DECAY
...
class YOLOv3Backbone(tf.keras.Model):
    def __init__(self):
        super().__init__(name="YOLOv3Backbone")
        regularizer = tf.keras.regularizers.l2(WEIGHT_DECAY)
    ...
    def call(self, inputs, training=False):
        ...
        c3 = self.stage3(x, training=training)
        c4 = self.stage4(c3, training=training)
        c5 = self.stage5(c4, training=training)
        return c3, c4, c5


def build_imagenet_classifier(num_classes=IMAGENET_NUM_CLASSES, name="YOLOv3Classifier"):
...
class YOLOv3(tf.keras.Model):
    def __init__(self, num_classes=NUM_CLASSES):
        super().__init__(name="YOLOv3")
        self.num_classes = num_classes
        regularizer = tf.keras.regularizers.l2(WEIGHT_DECAY)
        self.backbone = YOLOv3Backbone()
        self.p3 = tf.keras.layers.Conv2D(3 * (self.num_classes + 5), 1, padding="same", kernel_regularizer=regularizer)
        self.p4 = tf.keras.layers.Conv2D(3 * (self.num_classes + 5), 1, padding="same", kernel_regularizer=regularizer)
        self.p5 = tf.keras.layers.Conv2D(3 * (self.num_classes + 5), 1, padding="same", kernel_regularizer=regularizer)
    ...
    def _format_output(self, tensor, scale):
        tensor = tf.reshape(tensor, (-1, scale, scale, 3, self.num_classes + 5))
        class_logits = tensor[..., :self.num_classes]
        objectness = tf.sigmoid(tensor[..., self.num_classes: self.num_classes + 1])
        xy = tf.sigmoid(tensor[..., self.num_classes + 1: self.num_classes + 3])
        wh = tf.sigmoid(tensor[..., self.num_classes + 3:])
        return tf.concat([class_logits, objectness, xy, wh], axis=-1)

    def call(self, inputs, training=False):
        c3, c4, c5 = self.backbone(inputs, training=training)
        p_large = self._format_output(self.p5(c5, training=training), SCALES[0])
        p_medium = self._format_output(self.p4(c4, training=training), SCALES[1])
        p_small = self._format_output(self.p3(c3, training=training), SCALES[2])
        return {"yolo_s": p_large, "yolo_m": p_medium, "yolo_l": p_small}

    def _loss_for_scale(self, target, pred):
        ...
        return cls_loss + obj_loss + 5.0 * box_loss + 0.5 * noobj_loss

    def _total_loss(self, targets, preds):
        loss = self._loss_for_scale(targets["yolo_s"], preds["yolo_s"])
        loss += self._loss_for_scale(targets["yolo_m"], preds["yolo_m"])
        loss += self._loss_for_scale(targets["yolo_l"], preds["yolo_l"])
        reg = tf.add_n(self.losses) if self.losses else 0.0
        return loss / tf.cast(tf.shape(targets["yolo_s"])[0], tf.float32) + reg
    ...
    def detect(self, images):
        preds = self(images, training=False)
        anchors = [np.asarray(group, dtype=np.float32) / INPUT_SIZE[0] for group in ANCHORS]
        detections = []
        for batch_index in range(images.shape[0]):
            sample_detections = []
            for key, anchor_group in [("yolo_s", anchors[0]), ("yolo_m", anchors[1]), ("yolo_l", anchors[2])]:
                sample_pred = preds[key].numpy()[batch_index]
                scale = sample_pred.shape[0]
                ...
                for gy in range(scale):
                    for gx in range(scale):
                        for anchor_idx in range(3):
                            ...
                            tw, th = wh[gy, gx, anchor_idx] * anchor_group[anchor_idx]
                            cx = (gx + tx) / scale
                            cy = (gy + ty) / scale
...
def create_compiled_model(task_name="detector", dataset_name="coco"):
    if task_name == "classifier":
        ...
        model = build_imagenet_classifier(num_classes=num_classes, name="YOLOv3Classifier")
        model.compile(
