...
def draw_labelled_boxes(image, boxes, labels, names, *, normalized, prediction=False, max_draw=20):
    canvas = image.copy(); height, width = canvas.shape[:2]; color = (0, 220, 0) if prediction else (0, 0, 255)
    for box, label in list(zip(boxes, labels))[:max_draw]:
        class_id, score = label if prediction else (label, None); y1, x1, y2, x2 = box
        if normalized: x1, x2, y1, y2 = x1 * width, x2 * width, y1 * height, y2 * height
...
class DetectionInspectionCallback(tf.keras.callbacks.Callback):
    ...
    def _render(self, sample):
        ...
        canvas = draw_labelled_boxes(canvas, [d["box"] for d in detections], [(d["class_id"], d["score"]) for d in detections], self.class_names, normalized=self.pred_normalized, prediction=True)
        return cv2.resize(canvas, (320, 240), interpolation=cv2.INTER_AREA)
    def on_epoch_end(self, epoch, logs=None):
        ...
        grid = np.concatenate([np.concatenate(panels[row:row + 4], 1) for row in range(0, 16, 4)], 0); path = os.path.join(self.output_dir, f"epoch_{epoch + 1:03d}.png")
