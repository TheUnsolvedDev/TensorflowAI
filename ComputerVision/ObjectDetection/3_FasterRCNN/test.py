...
def draw_detections(image_bgr, detections):
    ...
    for det in detections[:MAX_DRAW_DETECTIONS]:
        ...
        cv2.putText(canvas, f"{label}: {score:.2f}", (x1, max(
            16, y1 - 4)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 220, 0), 1)
    return canvas
...
def main():
    ...
    if args.image:
        ...
        cv2.imwrite(os.path.join(INFERENCE_DIR, "single_pred.jpg"),
                    draw_detections(image_bgr, detections))
    else:
        dataset = FasterRCNNDataset(
            COCO_ROOT, TEST_SPLIT, augment=False, shuffle=False)
        for sample in dataset.samples[: args.num_images]:
            detections, image_bgr = infer_image(model, sample["image_path"])
            cv2.imwrite(os.path.join(
                INFERENCE_DIR, f"{sample['image_id']}_pred.jpg"), draw_detections(image_bgr, detections))
