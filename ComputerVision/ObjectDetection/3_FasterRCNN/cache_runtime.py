...
def _tree_stamp(path):
    ...
    for directory, _, names in os.walk(path):
        for name in sorted(names):
            ...
            digest.update(
                ("%d:%d" % (info.st_mtime_ns, info.st_size)).encode())
            count += 1
...
def fingerprint(detector, dataset_name, split, root, settings):
    ...
    payload = {"version": CACHE_TARGET_VERSION, "detector": detector,
               "dataset": dataset_name, "split": split, "root": _stamp(root), "settings": settings}
    if dataset_name == "coco":
        payload["annotations"] = _stamp(os.path.join(
            root, "annotations", "instances_%s.json" % split))
    else:
        payload["annotations"] = _tree_stamp(
            Path(root) / "Annotations" / "CLS-LOC" / split)
...
def cached_records(detector, dataset_name, split, root, settings, records, prepare=None):
    ...
    manifest_path, payload_path = os.path.join(
        active, "manifest.json"), os.path.join(active, "records.pkl")
    try:
    ...
    cap = max(0, int(os.environ.get(
        "DETECTOR_CACHE_MAX_SAMPLES", CACHE_MAX_SAMPLES)))
    if prepare and cap:
    ...
    try:
        ...
        manifest.update(fingerprint=digest, record_count=len(
            prepared), cache_max_samples=cap, cached_payload_records=min(cap, len(records)))
        ...
        _remove(previous)
        print("[cache] prepared active %s (%d records)" %
              (digest[:12], len(prepared)))
    finally:
        _remove(temporary)
    return prepared
