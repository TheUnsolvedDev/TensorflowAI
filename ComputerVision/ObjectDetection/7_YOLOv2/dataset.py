...
from config import ANCHORS, BATCH_SIZE, COCO_ROOT, GRID_SIZE, IMAGENET_BATCH_SIZE, IMAGENET_ROOT, IMAGENET_TRAIN_ANN_DIR, IMAGENET_TRAIN_DIR, IMAGENET_VAL_ANN_DIR, IMAGENET_VAL_DIR, INPUT_SIZE, MAX_BOXES, MIN_BOX_SIZE, TRAIN_SPLIT, VAL_SPLIT
...
def wh_iou(box_wh, anchor_wh):
    ...
    anchor_wh = np.asarray(anchor_wh, dtype=np.float32)
    inter = np.minimum(box_wh[0], anchor_wh[:, 0]) * np.minimum(box_wh[1], anchor_wh[:, 1])
    union = box_wh[0] * box_wh[1] + anchor_wh[:, 0] * anchor_wh[:, 1] - inter
...
def encode_target(boxes, labels, num_classes):
    target = np.zeros((GRID_SIZE, GRID_SIZE, NUM_ANCHORS, num_classes + 5), dtype=np.float32)
    ...
    for box, label in zip(boxes, labels):
        ...
        anchor_idx = int(np.argmax(wh_iou((bw, bh), anchors)))
        target[gy, gx, anchor_idx, :num_classes] = 0.0
        target[gy, gx, anchor_idx, label] = 1.0
        target[gy, gx, anchor_idx, num_classes: num_classes + 5] = [
            1.0,
            cx * GRID_SIZE - gx,
            cy * GRID_SIZE - gy,
            bw,
...
class YOLOv2Dataset:
    def __init__(self, coco_root=COCO_ROOT, split=TRAIN_SPLIT, batch_size=BATCH_SIZE, augment=False, shuffle=True, dataset_name="coco"):
        ...
        self.samples = cached_records("YOLOv2", dataset_name, split, coco_root, {"input_size": INPUT_SIZE, "grid_size": GRID_SIZE, "anchors": ANCHORS, "num_classes": self.num_classes, "max_boxes": MAX_BOXES}, self.samples, lambda records: [dict(sample, cache_target=encode_target(sample["boxes"], sample["labels"], self.num_classes), cache_boxes=pad_boxes(sample["boxes"], sample["labels"])) for sample in records])
        print(f"[YOLOv2Dataset] dataset={dataset_name} split={split} samples={len(self.samples)}")
    ...
    def _generator(self):
        ...
        for idx in indices:
            ...
            yield (
                ...
                sample.get("cache_target", encode_target(sample["boxes"], sample["labels"], self.num_classes)),
                *sample.get("cache_boxes", pad_boxes(sample["boxes"], sample["labels"])),
            )
    ...
    def build(self):
        output_signature = (
            ...
            tf.TensorSpec(shape=(GRID_SIZE, GRID_SIZE, NUM_ANCHORS, self.num_classes + 5), dtype=tf.float32),
...
def build_train_dataset(task_name="detector", dataset_name="coco", root=None, split=None):
    ...
    dataset = YOLOv2Dataset(coco_root=root or (IMAGENET_ROOT if dataset_name == "imagenet" else COCO_ROOT), split=split or ("train" if dataset_name == "imagenet" else TRAIN_SPLIT), augment=True, shuffle=True, dataset_name=dataset_name)
    return dataset, dataset.build()
...
def build_val_dataset(task_name="detector", dataset_name="coco", root=None, split=None):
    if task_name == "classifier":
        ...
        return dataset, dataset.build()
    dataset = YOLOv2Dataset(coco_root=root or (IMAGENET_ROOT if dataset_name == "imagenet" else COCO_ROOT), split=split or ("val" if dataset_name == "imagenet" else VAL_SPLIT), augment=False, shuffle=False, dataset_name=dataset_name)
    return dataset, dataset.build()
