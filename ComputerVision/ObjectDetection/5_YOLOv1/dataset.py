...
from config import BATCH_SIZE, COCO_ROOT, GRID_SIZE, IMAGENET_BATCH_SIZE, IMAGENET_ROOT, IMAGENET_TRAIN_ANN_DIR, IMAGENET_TRAIN_DIR, IMAGENET_VAL_ANN_DIR, IMAGENET_VAL_DIR, INPUT_SIZE, MAX_BOXES, MIN_BOX_SIZE, TRAIN_SPLIT, VAL_SPLIT, VOC_ROOT
...
def detection_num_classes(dataset_name):
    if dataset_name == "imagenet": return IMAGENET_DET_NUM_CLASSES
    if dataset_name == "voc": return len(VOC_CLASSES)
    return NUM_CLASSES


def encode_target(boxes, labels, num_classes):
    target = np.zeros((GRID_SIZE, GRID_SIZE, num_classes + 5), dtype=np.float32)
    for box, label in zip(boxes, labels):
        ...
        local_x = cx * GRID_SIZE - grid_x
        local_y = cy * GRID_SIZE - grid_y
        width = np.clip(x2 - x1, 1e-6, 1.0)
        height = np.clip(y2 - y1, 1e-6, 1.0)
        if target[grid_y, grid_x, num_classes] < 0.5:
            target[grid_y, grid_x, :num_classes] = 0.0
            target[grid_y, grid_x, label] = 1.0
            target[grid_y, grid_x, num_classes: num_classes + 5] = [1.0, local_x, local_y, width, height]
    return target


def pad_boxes(boxes, labels):
...
def collect_imagenet_detection_samples(imagenet_root=IMAGENET_ROOT, split="train"):
    ...
    if split == "train":
        ...
        for class_name in class_names:
            class_ann_dir = ann_root / class_name
            class_image_dir = image_root / class_name
            for xml_path in sorted(class_ann_dir.glob("*.xml")):
                image_path = class_image_dir / f"{xml_path.stem}.JPEG"
...
class YOLOv1Dataset:
    def __init__(self, coco_root=COCO_ROOT, split=TRAIN_SPLIT, batch_size=BATCH_SIZE, augment=False, shuffle=True, dataset_name="coco"):
        ...
        if dataset_name == "imagenet":
            self.samples = collect_imagenet_detection_samples(imagenet_root=coco_root, split="train" if split == "train" else "val")
        elif dataset_name == "voc":
            self.samples = voc_records(coco_root, split, MIN_BOX_SIZE)
        else:
            self.samples = collect_coco_samples(coco_root, split)
        self.samples = cached_records("YOLOv1", dataset_name, split, coco_root, {"input_size": INPUT_SIZE, "grid_size": GRID_SIZE, "num_classes": self.num_classes, "max_boxes": MAX_BOXES}, self.samples, lambda records: [dict(sample, cache_target=encode_target(sample["boxes"], sample["labels"], self.num_classes), cache_boxes=pad_boxes(sample["boxes"], sample["labels"])) for sample in records])
        print(f"[YOLOv1Dataset] dataset={dataset_name} split={split} samples={len(self.samples)}")
    ...
    def _generator(self):
        ...
        for idx in indices:
            ...
            yield (
                ...
                sample.get("cache_target", encode_target(boxes, labels, self.num_classes)),
                *sample.get("cache_boxes", pad_boxes(boxes, labels)),
            )

    def _process(self, image_path, target, boxes, labels):
        ...
        return image, {"yolo_output": target, "gt_boxes": boxes, "gt_labels": labels}

    def build(self):
        output_signature = (
            tf.TensorSpec(shape=(), dtype=tf.string),
            tf.TensorSpec(shape=(GRID_SIZE, GRID_SIZE, self.num_classes + 5), dtype=tf.float32),
...
def build_train_dataset(task_name="detector", dataset_name="coco", root=None, split=None):
    ...
    dataset = YOLOv1Dataset(coco_root=root or (IMAGENET_ROOT if dataset_name == "imagenet" else VOC_ROOT if dataset_name == "voc" else COCO_ROOT), split=split or ("train" if dataset_name in ("imagenet", "voc") else TRAIN_SPLIT), augment=True, shuffle=True, dataset_name=dataset_name)
    return dataset, dataset.build()
...
def build_val_dataset(task_name="detector", dataset_name="coco", root=None, split=None):
    if task_name == "classifier":
        ...
        return dataset, dataset.build()
    dataset = YOLOv1Dataset(coco_root=root or (IMAGENET_ROOT if dataset_name == "imagenet" else VOC_ROOT if dataset_name == "voc" else COCO_ROOT), split=split or ("val" if dataset_name in ("imagenet", "voc") else VAL_SPLIT), augment=False, shuffle=False, dataset_name=dataset_name)
    return dataset, dataset.build()
