...
from config import GRID_SIZE, IMAGENET_NUM_CLASSES, INPUT_SIZE, LEARNING_RATE, MAX_DETECTIONS, NMS_IOU_THRESHOLD, SCORE_THRESHOLD, WEIGHT_DECAY
...
class ResidualBackbone(tf.keras.Model):
    def __init__(self):
        super().__init__(name="YOLOv1Backbone")
        regularizer = tf.keras.regularizers.l2(WEIGHT_DECAY)
        ...
        self.stage2 = ResidualStage(64, blocks=2, stride=1)
        self.stage3 = ResidualStage(128, blocks=2, stride=2)
        self.stage4 = ResidualStage(256, blocks=2, stride=2)
        self.stage5 = ResidualStage(512, blocks=2, stride=2)
        self.stage6 = ResidualStage(512, blocks=2, stride=2)
    ...
    def call(self, inputs, training=False):
        ...
        x = self.stage6(x, training=training)
        return x


def build_imagenet_classifier(num_classes=IMAGENET_NUM_CLASSES, name="YOLOv1Classifier"):
...
class YOLOv1(tf.keras.Model):
    def __init__(self, num_classes=NUM_CLASSES):
        super().__init__(name="YOLOv1")
        self.num_classes = num_classes
        self.backbone = ResidualBackbone()
        regularizer = tf.keras.regularizers.l2(WEIGHT_DECAY)
        self.head = tf.keras.Sequential(
            [
                tf.keras.layers.Conv2D(512, 3, padding="same", activation="relu", kernel_regularizer=regularizer),
                tf.keras.layers.Conv2D(self.num_classes + 5, 1, padding="same", kernel_regularizer=regularizer),
            ]
        )
    ...
    def call(self, inputs, training=False):
        ...
        outputs = tf.reshape(outputs, (-1, GRID_SIZE, GRID_SIZE, self.num_classes + 5))
        class_logits = outputs[..., :self.num_classes]
        objectness = tf.sigmoid(outputs[..., self.num_classes: self.num_classes + 1])
        box_params = tf.sigmoid(outputs[..., self.num_classes + 1:])
        return tf.concat([class_logits, objectness, box_params], axis=-1, name="yolo_output")

    def compute_total_loss(self, target, pred):
        target = tf.cast(target, tf.float32)
        pred = tf.cast(pred, tf.float32)
        target_classes = target[..., :self.num_classes]
        ...
        cls_loss = tf.nn.softmax_cross_entropy_with_logits(labels=target_classes, logits=pred_classes)
        cls_loss = tf.reduce_sum(cls_loss * tf.squeeze(target_obj, axis=-1))
    ...
    def detect(self, images):
        ...
        for sample_pred in preds:
            ...
            box_params = sample_pred[..., self.num_classes + 1:]
            for gy in range(GRID_SIZE):
                for gx in range(GRID_SIZE):
                    score = float(objectness[gy, gx])
                    if score < SCORE_THRESHOLD:
                        continue
                    cls = int(np.argmax(class_probs[gy, gx])) + 1
                    cls_score = float(class_probs[gy, gx, cls - 1] * score)
                    if cls_score < SCORE_THRESHOLD:
                        continue
                    tx, ty, tw, th = box_params[gy, gx]
                    cx = (gx + tx) / GRID_SIZE
                    cy = (gy + ty) / GRID_SIZE
...
def create_compiled_model(task_name="detector", dataset_name="coco"):
    if task_name == "classifier":
        ...
        model = build_imagenet_classifier(num_classes=num_classes, name="YOLOv1Classifier")
        model.compile(
    ...
    # Materialize variables with a real forward call; `Model.build()` alone
    # only marks this custom trainer built and leaves nested layers unbuilt.
