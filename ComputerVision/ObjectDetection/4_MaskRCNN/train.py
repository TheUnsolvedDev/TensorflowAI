...
class CompactProgress(tf.keras.callbacks.Callback):
    def on_epoch_begin(self, epoch, logs=None):
        self.epoch, self.started, self.steps = epoch + 1, time.monotonic(), self.params.get("steps")
    def on_train_batch_end(self, batch, logs=None):
        step = batch + 1
        if step % 10 and step != self.steps: return
        loss = float((logs or {}).get("loss", float("nan"))); elapsed = time.monotonic() - self.started
...
def fit_task(strategy, task_name, dataset_name, continue_training, epochs, initialize_from=None, reset=False, full_reports=False):
...
def main():
    ...
    parser.add_argument("--full-reports", action="store_true", help="Enable expensive reports; the labelled 4x4 mask inspection grid is written every detector epoch.")
    ...
    if args.classifier is not None:
        classifier_checkpoint = fit_task(strategy, "classifier", args.classifier, args.continue_training, args.epochs or CLASSIFIER_EPOCHS, reset=args.reset, full_reports=args.full_reports)
    if args.detector is not None:
        fit_task(strategy, "detector", args.detector, args.continue_training, args.epochs or EPOCHS, initialize_from=classifier_checkpoint, reset=args.reset, full_reports=args.full_reports)
