...
def build_encoder(encoder_embedding, encoder_units, decoder_units, num_encoder):
    ...
    for i in range(num_encoder):
        encoder_x, forward_h, forward_c, backward_h, backward_c = tf.keras.layers.Bidirectional(
            tf.keras.layers.LSTM(
                encoder_units, return_sequences=True, return_state=True),
            name=f"encoder_lstm_{i+1}"
        )(encoder_x)
    state_h = tf.keras.layers.Concatenate(
        name="encoder_state_h")([forward_h, backward_h])
    state_c = tf.keras.layers.Concatenate(
        name="encoder_state_c")([forward_c, backward_c])
    state_h = tf.keras.layers.Dense(
        decoder_units, activation="tanh", name="encoder_to_decoder_h")(state_h)
    state_c = tf.keras.layers.Dense(
        decoder_units, activation="tanh", name="encoder_to_decoder_c")(state_c)
...
def build_seq2seq_model(source_vocab_size, target_vocab_size, source_max_length=128, target_max_length=128, embedding_dim=512, encoder_units=256, decoder_units=512, num_encoder=2, num_decoder=1):
    ...
    decoder_outputs = tf.keras.layers.Dense(
        target_vocab_size, activation="softmax", name="output_projection")(decoder_outputs)
    return tf.keras.Model(inputs=[encoder_inputs, decoder_inputs], outputs=decoder_outputs, name="deep_seq2seq_model")
