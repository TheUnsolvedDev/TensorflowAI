...
from model import build_coverage_seq2seq_model
...
def masked_loss(y_true, y_pred):
    loss = tf.keras.losses.sparse_categorical_crossentropy(y_true, y_pred); mask = tf.cast(tf.not_equal(y_true, 0), loss.dtype)
    return tf.math.divide_no_nan(tf.reduce_sum(loss * mask), tf.reduce_sum(mask))
def masked_accuracy(y_true, y_pred):
    correct = tf.cast(tf.equal(y_true, tf.argmax(y_pred, axis=-1, output_type=y_true.dtype)), tf.float32); mask = tf.cast(tf.not_equal(y_true, 0), tf.float32)
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--dataset", choices=PATHS, default="manythings_english_french"); parser.add_argument("--epochs", type=int, default=EPOCHS)
    parser.add_argument("--steps-per-epoch", type=int, default=None); parser.add_argument("--validation-steps", type=int, default=None); parser.add_argument("--smoke", action="store_true"); parser.add_argument("--resume", action="store_true"); parser.add_argument("--gpu", type=int, default=-1)
    ...
    train_ds, val_ds = data.load_data(); default_train_steps = data.get_steps_per_epoch(True); default_validation_steps = data.get_steps_per_epoch(False); log_dir = os.path.join(LOG_DIR, args.dataset, "CoverageModel"); os.makedirs(log_dir, exist_ok=True)
    with strategy.scope():
        model = build_coverage_seq2seq_model(data.get_source_vocab_size(), data.get_target_vocab_size(), SOURCE_MAX_LENGTH, TARGET_MAX_LENGTH, EMBEDDING_DIM, ENCODER_UNITS)
