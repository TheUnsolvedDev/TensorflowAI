...
def get_log_dir(dataset_name):
    return f"logs/{dataset_name}/StackedRNN"
...
def main():
    ...
    try: strategy = tf.distribute.MirroredStrategy(cross_device_ops=tf.distribute.NcclAllReduce())
    except (RuntimeError, ValueError): strategy = tf.distribute.MirroredStrategy()
    ...
    with strategy.scope():
        model = build_stacked_rnn(vocab_size=dataset.get_vocab_size(
