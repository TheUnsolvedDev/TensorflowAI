    ...
    def _train_step(self, obs, actions, rewards, next_obs, dones, next_mask):
        ...
        def step_fn(obs, actions, rewards, next_obs, dones, next_mask):
            with tf.GradientTape() as tape:
                ...
                neg_inf = tf.constant(-1e9, dtype=next_q.dtype)
                next_q_masked = tf.where(next_mask > 0, next_q, neg_inf)
                next_q_max = tf.reduce_max(next_q_masked, axis=-1)
        ...
        per_replica_loss = self.strategy.run(
            step_fn,
            args=(obs, actions, rewards, next_obs, dones, next_mask)
        )
