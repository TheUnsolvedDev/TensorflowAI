...
import env.mario_env as mario_env
...
def latest_weights(model_name, requested=None):
    ...
    latest = checkpoints[-1] if checkpoints else directory / "mario.weights.h5"
    return str(latest)
...
def main():
    ...
    parser.add_argument("--level", default="SuperMarioBros-Nes-v0")
    ...
    if args.visualize:
        ...
        probe = wrappers.training_env(game=args.level, render_mode="human")
        ...
        evaluate.test_agent(
            agent,
            args.episodes,
            lambda: wrappers.training_env(game=args.level, render_mode="human"),
        )
        return
    ...
    make_env = functools.partial(mario_env.MarioEnv, game=args.level)
