...
def train():
    ...
    writer = tf.summary.create_file_writer(f"{LOG_DIR}/{timestamp}")
    ...
    for episode in tqdm.tqdm(range(EPOCHS), desc="Training Episodes"):
        ...
        agent.update(tf.convert_to_tensor(obs_arr), tf.convert_to_tensor(act_arr), tf.convert_to_tensor(ret_arr))
        ...
        if total_reward > best_reward:
            ...
            while not record_done:
                record_action = agent.get_action(record_obs)
                record_obs, _, term, trunc, _ = record_env.step(record_action)
                record_done = term or trunc
            record_env.close()
