...
def make_env(record=False, episode_id=0):
    if record: return gym.wrappers.RecordVideo(gym.make(ENV_NAME, render_mode="rgb_array"), VIDEO_DIR, episode_trigger=lambda _: True, name_prefix=f"episode_{episode_id}")
    return gym.make(ENV_NAME)

def train():
    agent = REINFORCEAgent(); checkpoint_dir = "checkpoints"; os.makedirs(checkpoint_dir, exist_ok=True); writer = tf.summary.create_file_writer(f"{LOG_DIR}/{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"); best_reward = -float("inf")
    for episode in tqdm.tqdm(range(EPOCHS), desc="Training Episodes"):
        ...
        while not done:
            action = agent.get_action(obs); next_obs, reward, term, trunc, _ = env.step(action); done = term or trunc; buffer.store(obs, action, reward); obs = next_obs; total_reward += reward
        ...
        if total_reward > best_reward:
            best_reward = total_reward; agent.save(f"{checkpoint_dir}/best.weights.h5"); record_env = make_env(True, episode); record_obs, _ = record_env.reset(); record_done = False
