    ...
    def distributed_train_step(self, obs, actions, rewards, next_obs, dones):
        ...
        def step_fn(obs, actions, rewards, next_obs, dones):
            with tf.GradientTape() as tape:
                ...
                next_online_q = self.q_net(next_obs, training=False)
                next_actions = tf.argmax(next_online_q, axis=-1, output_type=tf.int32)
                next_action_mask = tf.one_hot(next_actions, self.action_dim, dtype=tf.float32)
                next_target_q = self.target_net(next_obs, training=False)
                target_next_q = tf.reduce_sum(next_target_q * next_action_mask, axis=-1)
        ...
        per_replica_loss = self.strategy.run(step_fn, args=(obs, actions, rewards, next_obs, dones))
